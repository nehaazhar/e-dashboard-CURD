...
const ProductList = () => {
  ...
  useEffect(() => {
        fetch("http://localhost:5100/companies", {
            headers: {
                authorization: `bearer ${JSON.parse(localStorage.getItem("token"))}`
            }
        })
    ...
    const getProducts = async () => {
        ...
        const url = `http://localhost:5100/products?${params.toString()}`;
    ...
    const deleteProduct = async () => {
        ...
        try {
            let response = await fetch(
                `http://localhost:5100/product/${deleteId}`,
                {
                    method: "DELETE",
                    headers: {
                        authorization: `Bearer ${JSON.parse(localStorage.getItem("token"))}`
    ...
    const searchProducts = async (key) => {
        try {
            let response = await fetch(`http://localhost:5100/search/${key}`, {
                headers: {
                    authorization: `Bearer ${JSON.parse(localStorage.getItem('token'))}`
                }
            });
        ...
        <div className="product-list">
            ...
            <div className="top-bar">
                <div className="search-container">
                    <input type="text" value={searchText} onChange={(e) => setSearchText(e.target.value)} placeholder="Search product..." className="search-box" id="searchInput" />
